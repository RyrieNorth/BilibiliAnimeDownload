...
##定义番剧ID号
AnimeID = "ss42135"             ##辉夜：ss41411，实教：ss42135

##系统代理，这里我用的是v2ray的网关代理
proxies = {
 "https": None,
...
if AnimeType == "ep":
    ...
    ##拼接对应番剧的URL
    AnimeListURL = f"http://api.bilibili.com/pgc/view/web/season?" + "ep_id=" + AnimeID.replace("ep","")

elif AnimeType == "ss":
    print ("你输入的番剧类型为Ss类型")

    ##拼接对应番剧的URL
    AnimeListURL = f"http://api.bilibili.com/pgc/view/web/season?" + "season_id=" + AnimeID.replace("ss","")
...
if AnimeRes.status_code == 200:
    ...
    AnimeStatesCode = Animejs.get('code')
    if AnimeStatesCode == -404:
        print ("你输入的番剧ID有误，或该番剧属于港澳台类型，请尝试设置系统代理")
    elif AnimeStatesCode == 0:
        print ("你输入的番剧ID有效，正在获取番剧地址")
        ...
        for i in Animejs2:
            ...
            ##获取视频Cid
            VideoRes = requests.get(url=VideoListURL,headers=headers,proxies=proxies)
